...
namespace traciSharp
{
    public class CommandProcessor : Query
    {
        ...
        public CommandProcessor(Socket socket) : base(socket)
        {
        ...
        protected static String verifyGetVarResponse(Command command, int id, int content, String contentStr)
        {
            verify("response code", Convert.ToInt32(id), Convert.ToInt32(command.Id()));
            verify("variable ID", Convert.ToInt32(content), Convert.ToInt32(command.Content().readUnsignedByte()));
            String str = command.Content().readStringASCII();
            if (contentStr != null) {
                verify("object ID", contentStr, str);
            }
            return str;
        }
    }
